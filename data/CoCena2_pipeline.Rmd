...
```{r}
working_directory = paste0(setwd(dirname(rstudioapi::getSourceEditorContext()$path)),"/CoCena_folders/")
topvar_genes = 5000
voi = "condition"
...
print_distribution_plots = F
min_nodes_number_for_network=50 
min_nodes_number_for_cluster=65
data_in_log=T
range_GFC=2.0
...
```{r}
#diets----
#info_dataset <- anno_diet
# annotation_table<-"diet_"
# annotation_table<-"diet_rlog"
#annotation_table<-"diet_rlog_unblind"

# count_table <- removedbatch_dds_diet_rlog
#count_table <- removedbatch_dds_diet_rlog_unblind[,!grepl("mean",colnames(removedbatch_dds_diet_rlog_unblind))]
# col_clust<-c("GFC_Creld2 +/+_CD","GFC_Creld2 KO/KO_CD",
#              "GFC_Creld2 +/+_HFD","GFC_Creld2 KO/KO_HFD",
# "GFC_Creld2 +/+_Recovery","GFC_Creld2 KO/KO_Recovery")
#drug----
#info_dataset <- anno_drug
# annotation_table<-"drug"
# annotation_table<-"drugrlog"
#annotation_table<-"drugrlog_unblind"

# count_table <- removedbatch_dds_drug_rlog
#count_table <- removedbatch_dds_drug_rlog_unblind[,!grepl("mean",colnames(removedbatch_dds_drug_rlog_unblind))]
# 
# col_clust<-c("GFC_Creld2 +/+_Sucrose","GFC_Creld2 KO/KO_Sucrose",
#              "GFC_Creld2 +/+_Tg","GFC_Creld2 KO/KO_Tg",
#              "GFC_Creld2 +/+_Tm","GFC_Creld2 KO/KO_Tm")

#drug s+Tm----
info_dataset <- anno_drug_s_tm
# # annotation_table<-"drug_s_tm"
# # annotation_table<-"drug_s_tm_rlog"
annotation_table<-"drug_s_tm_rlog_unblind"
# 
# # count_table <- removedbatch_dds_drug_s_tm_rlog
count_table <- removedbatch_dds_drug_s_tm_rlog_unblind[,!grepl("mean",colnames(removedbatch_dds_drug_s_tm_rlog_unblind))]
# 
col_clust<-c("GFC_Creld2 +/+_Sucrose","GFC_Creld2 KO/KO_Sucrose","GFC_Creld2 +/+_Tm","GFC_Creld2 KO/KO_Tm")#drug_s_tm



#possibly wrong count_table inputs----
# info_dataset <- anno_diet
# count_table <- norm_anno_diet

# info_dataset <- anno_drug
# count_table <- norm_anno_drug

# info_dataset <- anno_drug_s_tm
# count_table <- norm_anno_drug_s_tm
#----

count_table$GENEID <- NULL
count_table$GENETYPE <- NULL
count_table$DESCRIPTION <- NULL
count_table$CHR <- NULL
#take the mean if symbol is duplicated
count_table <- count_table %>% group_by(SYMBOL) %>% summarise_all(funs(mean))
count_table <- as.data.frame(count_table)
row.names(count_table) <- count_table$SYMBOL
...
# optimal_cutoff = 0.706
optimal_cutoff= 0.659
cutoff_wd <- paste0(format(Sys.time(), '%Y-%m-%d'), annotation_table,"_",optimal_cutoff, "_", topvar_genes,"_GFC_range",range_GFC)
...
#GFC_all_genes <- GFC_calculation(voi_id = "ID")
...
heatmap_cluster <- heatmap_clusters(data = cluster_information, cluster_cols = F,order_col = col_clust )
heatmap_cluster_original <- heatmap_clusters(data = cluster_information, cluster_cols = T)

```
```{r}
cluster_information_drug_s_tm<-cluster_information
write.xlsx(cluster_information[cluster_information$cluster_included=="yes",],file = paste0(getwd(), "/CoCena_folders/",Sys.Date(),"_drug_s_tm_cluster_information.xlsx")
)
#cluster_information_diet<-cluster_information
#cluster_information_diet[!cluster_information_diet$color=="white",c(1:2,5)]
...
source(paste0(working_directory,"scripts/clusterprofiler_autoCena_N.R" ))
...
```{r}
cluster_data = cluster_information %>% dplyr::filter(cluster_included=="yes")
corresp_info = info_dataset[rownames(dd2)%in%rownames(info_dataset),]
annotation_table$merged = purrr::pmap(corresp_info[intersect(condition,colnames(info_dataset))], paste, sep="-") %>% unlist()
```
## Supplementary table

```{r}
#check the cluster names and number of clusters
# drug_s_tm_clusters <-cluster_information[cluster_information$cluster_included=="yes",]
library("openxlsx")
wb<-createWorkbook()
...
for(i in 1:nrow(cluster_data)){
  row <- cluster_data[i,]
  color <- row$color
  number_of_genes <- row$gene_no
  data <- as.data.frame(cluster_data$gene_n[cluster_data$color==color])
  colnames(data) <- "x"
  cluster_cols <- str_split_fixed(data$x,",",number_of_genes)
  cluster_rows <- t(cluster_cols)
  cluster_GFC <- GFC_all_genes[GFC_all_genes$Gene %in% cluster_rows,]
  cluster_counts <- removedbatch_dds_drug_s_tm_rlog_unblind[removedbatch_dds_drug_s_tm_rlog_unblind$SYMBOL %in% cluster_rows, ]
  cluster <- merge(cluster_GFC,cluster_counts, by.x="Gene", by.y="SYMBOL" )
cluster[,c(1,8,11:13,2:7,14:19, 9:13)]
addWorksheet(wb = wb, sheetName = color, gridLines = FALSE)
writeDataTable(wb = wb, sheet = color, x = cluster)
  
}
saveWorkbook(wb, "supplementary_drug_loop.xls", overwrite = TRUE)
```


```{r}
#check the cluster names and number of clusters
diet_clusters <-cluster_information[cluster_information$cluster_included=="yes",]
```
#Diet clusters
go in the clusters one by one: pink
```{r}
pink <- as.data.frame(cluster_information_diet$gene_n[cluster_information_diet$color=="pink"])
colnames(pink) <- c("x")
 
library(stringr)
pink_cols <- str_split_fixed(pink$x, ",", 557)
pink_rows <- t(pink_cols)

pink_GFC <- GFC_all_genes[GFC_all_genes$Gene %in% pink_rows, ]
pink_counts <- removedbatch_dds_diet_rlog[removedbatch_dds_diet_rlog$SYMBOL %in% pink_rows, ]
pink <- merge(pink_GFC,pink_counts, by.x="Gene", by.y="SYMBOL" )
pink[,c(1,8,11:13,2:7,14:19, 9:13)]
```
go in the clusters one by one: orchid
```{r}
orchid <- as.data.frame(cluster_information_diet$gene_n[cluster_information_diet$color=="orchid"])
colnames(orchid) <- c("x")
 
library(stringr)
orchid_cols <- str_split_fixed(orchid$x, ",", 65)
orchid_rows <- t(orchid_cols)

orchid_GFC <- GFC_all_genes[GFC_all_genes$Gene %in% orchid_rows, ]
orchid_counts <- removedbatch_dds_diet_rlog[removedbatch_dds_diet_rlog$SYMBOL %in% orchid_rows, ]
orchid <- merge(orchid_GFC,orchid_counts, by.x="Gene", by.y="SYMBOL" )
orchid[,c(1,8,11:13,2:7,14:19, 9:13)]
```
go in the clusters one by one: maroon
```{r}
maroon <- as.data.frame(cluster_information_diet$gene_n[cluster_information_diet$color=="maroon"])
colnames(maroon) <- c("x")
 
library(stringr)
maroon_cols <- str_split_fixed(maroon$x, ",", 141)
maroon_rows <- t(maroon_cols)

maroon_GFC <- GFC_all_genes[GFC_all_genes$Gene %in% maroon_rows, ]
maroon_counts <- removedbatch_dds_diet_rlog[removedbatch_dds_diet_rlog$SYMBOL %in% maroon_rows, ]
maroon <- merge(maroon_GFC,maroon_counts, by.x="Gene", by.y="SYMBOL" )
maroon[,c(1,8,11:13,2:7,14:19, 9:13)]
```

go in the clusters one by one: darkgreen
```{r}
darkgreen <- as.data.frame(cluster_information_diet$gene_n[cluster_information_diet$color=="darkgreen"])
colnames(darkgreen) <- c("x")
 
library(stringr)
darkgreen_cols <- str_split_fixed(darkgreen$x, ",", 558)
darkgreen_rows <- t(darkgreen_cols)

darkgreen_GFC <- GFC_all_genes[GFC_all_genes$Gene %in% darkgreen_rows, ]
darkgreen_counts <- removedbatch_dds_diet_rlog[removedbatch_dds_diet_rlog$SYMBOL %in% darkgreen_rows, ]
darkgreen <- merge(darkgreen_GFC,darkgreen_counts, by.x="Gene", by.y="SYMBOL" )
darkgreen[,c(1,8,11:13,2:7,14:19, 9:13)]
```

go in the clusters one by one: darkorange
```{r}
darkorange <- as.data.frame(cluster_information_diet$gene_n[cluster_information_diet$color=="darkorange"])
colnames(darkorange) <- c("x")
 
library(stringr)
darkorange_cols <- str_split_fixed(darkorange$x, ",", 87)
darkorange_rows <- t(darkorange_cols)

darkorange_GFC <- GFC_all_genes[GFC_all_genes$Gene %in% darkorange_rows, ]
darkorange_counts <- removedbatch_dds_diet_rlog[removedbatch_dds_diet_rlog$SYMBOL %in% darkorange_rows, ]
darkorange <- merge(darkorange_GFC,darkorange_counts, by.x="Gene", by.y="SYMBOL" )
darkorange[,c(1,8,11:13,2:7,14:19, 9:13)]
```

go in the clusters one by one: darkgrey
```{r}
darkgrey <- as.data.frame(cluster_information_diet$gene_n[cluster_information_diet$color=="darkgrey"])
colnames(darkgrey) <- c("x")
 
library(stringr)
darkgrey_cols <- str_split_fixed(darkgrey$x, ",", 73)
darkgrey_rows <- t(darkgrey_cols)

darkgrey_GFC <- GFC_all_genes[GFC_all_genes$Gene %in% darkgrey_rows, ]
darkgrey_counts <- removedbatch_dds_diet_rlog[removedbatch_dds_diet_rlog$SYMBOL %in% darkgrey_rows, ]
darkgrey <- merge(darkgrey_GFC,darkgrey_counts, by.x="Gene", by.y="SYMBOL" )
darkgrey[,c(1,8,11:13,2:7,14:19, 9:13)]
```

go in the clusters one by one: gold
```{r}
gold <- as.data.frame(cluster_information_diet$gene_n[cluster_information_diet$color=="gold"])
colnames(gold) <- c("x")
 
library(stringr)
gold_cols <- str_split_fixed(gold$x, ",", 721)
gold_rows <- t(gold_cols)

gold_GFC <- GFC_all_genes[GFC_all_genes$Gene %in% gold_rows, ]
gold_counts <- removedbatch_dds_diet_rlog[removedbatch_dds_diet_rlog$SYMBOL %in% gold_rows, ]
gold <- merge(gold_GFC,gold_counts, by.x="Gene", by.y="SYMBOL" )
gold[,c(1,8,11:13,2:7,14:19, 9:13)]
```

go in the clusters one by one: steelblue
```{r}
steelblue <- as.data.frame(cluster_information_diet$gene_n[cluster_information_diet$color=="steelblue"])
colnames(steelblue) <- c("x")
 
library(stringr)
steelblue_cols <- str_split_fixed(steelblue$x, ",", 459)
steelblue_rows <- t(steelblue_cols)

steelblue_GFC <- GFC_all_genes[GFC_all_genes$Gene %in% steelblue_rows, ]
steelblue_counts <- removedbatch_dds_diet_rlog[removedbatch_dds_diet_rlog$SYMBOL %in% steelblue_rows, ]
steelblue <- merge(steelblue_GFC,steelblue_counts, by.x="Gene", by.y="SYMBOL" )
steelblue[,c(1,8,11:13,2:7,14:19, 9:13)]
```

go in the clusters one by one: indianred
```{r}
indianred <- as.data.frame(cluster_information_diet$gene_n[cluster_information_diet$color=="indianred"])
colnames(indianred) <- c("x")
 
library(stringr)
indianred_cols <- str_split_fixed(indianred$x, ",", 64)
indianred_rows <- t(indianred_cols)

indianred_GFC <- GFC_all_genes[GFC_all_genes$Gene %in% indianred_rows, ]
indianred_counts <- removedbatch_dds_diet_rlog[removedbatch_dds_diet_rlog$SYMBOL %in% indianred_rows, ]
indianred <- merge(indianred_GFC,indianred_counts, by.x="Gene", by.y="SYMBOL" )
indianred[,c(1,8,11:13,2:7,14:19, 9:13)]
```
#write cluster info into excel
```{r}
library("openxlsx")
wb<-createWorkbook()
addWorksheet(wb = wb, sheetName = "orchid", gridLines = FALSE)
writeDataTable(wb = wb, sheet = 1, x = orchid)
addWorksheet(wb = wb, sheetName = "maroon", gridLines = FALSE)
writeData(wb = wb, sheet = 2, x = maroon)
addWorksheet(wb = wb, sheetName = "darkgreen", gridLines = FALSE)
writeData(wb = wb, sheet = 3, x = darkgreen)

addWorksheet(wb = wb, sheetName = "darkgrey", gridLines = FALSE)
writeData(wb = wb, sheet = 3, x = darkgrey)

addWorksheet(wb = wb, sheetName = "darkorange", gridLines = FALSE)
writeData(wb = wb, sheet = 3, x = darkorange)

addWorksheet(wb = wb, sheetName = "gold", gridLines = FALSE)
writeData(wb = wb, sheet = 3, x = gold)

addWorksheet(wb = wb, sheetName = "steelblue", gridLines = FALSE)
writeData(wb = wb, sheet = 3, x = steelblue)

addWorksheet(wb = wb, sheetName = "indianred", gridLines = FALSE)
writeData(wb = wb, sheet = 3, x = indianred)

addWorksheet(wb = wb, sheetName = "pink", gridLines = FALSE)
writeData(wb = wb, sheet = 3, x = pink)
saveWorkbook(wb, "supplementary_diet.xls", overwrite = TRUE)
...
heatmap_insert_new_clustering(heatmap_data = cluster_table, height = height, compare_column_left = "Genotype", middle = "Experiment", compare_column_right = "new_cluster", voi_id=voi_id,  color = "new_cluster")
```
...
correlation<-function(type_of_correlation=c("pearson","spearman"),pVal_treshold=0.05,save_raw_data=FALSE){
  ...
  ##funktioniert mit gro�em datensatz! flattencormatrix das problem?
...
GFC_calculation<-function(normdata=original_data,
  ...
  if(length(group)>1){
    tmp_condition<-data.frame(1:nrow(info_Dataset))
    for(column in group){
      tmp_condition[,column]<-info_Dataset[,column]
    }
    rownames(tmp)<-rownames(info_Dataset)
    tmp_condition$condition<-apply(tmp_condition,1,function(x){
      paste0(x,collapse = "-")
    })
    norm_data_anno$GFC_group<- tmp_condition$condition ##eventuell mergen um fehler vorzubeugen ??
  }else{
...
toCytoscape_all<-function(cluster_information=cluster_information){
  ...
  cluster_information_df$TF_list <- apply(cluster_information_df , 1 , function(x) {
    if(x["Gene"] %in% TF_list[,organism]) {                                       
      tmp <- TF_list[grepl(paste0("^",x["Gene"],"$") , TF_list[,organism]) , ]
...
plot_GFC_networks<-function(igraph_object=igraph_object,print_to_pdf=FALSE,print_edges_png_nodes_pdf=c("each","one","none")){
  ...
  for(col in colnames(GFC_all_genes_color)[grepl("GFC", colnames(GFC_all_genes_color))]){
    ...
    plot <- ggplot2::ggplot(GFC_all_genes_color,aes(x = V1 , y = V2))+
      geom_point(aes(color = as.numeric(GFC_all_genes_color[,col])))+
      scale_colour_gradientn(colours = c("red","white", "blue"),
                             limits = c(max(GFC_all_genes_color[,col]) , min(GFC_all_genes_color[,col])))
...
plotFunction<-function(){
  p<- ggplot(TFs_matrix_all_plot.melt.mean,aes(x=variable,y=mean,fill=condition)) +
    geom_bar(stat="identity",position = "dodge") +
    facet_wrap(~variable,scales = "free")
  print(p)
}
#clusterprofiler
clusterprofiler_autoCena<-function(cluster_to_check=c("all",cluster_color),group=c("condition")){
  summary<-allargs()
  cluster_available<-unique(cluster_information$color)
  df<-data.frame()
  no<-0
  biggest_cluster <- cluster_information[grepl(paste0("^" , names(which.max(table(cluster_information$color))) , "$") , cluster_information$color) , ]
  df <- data.frame(longest_cluster = as.character(cluster_information[cluster_information$color == unique(biggest_cluster$try) , 2]))
  
  if(cluster_to_check==c("all")){
    for(Thomas in cluster_available) {
      no <- 1 + no
      x <- as.character(Thomas)
      new.col <- as.character(cluster_information[cluster_information$color == x , 4])
  ...
  ###cluster Profiler
  normdata$condition<-info_Dataset$merged
  universe <- rownames(count_table)
...
```
    ...
    #Hallmark gene sets
    #start
    #egmt <- enricher(module_entrez, TERM2GENE=c1_hallmark, universe = universe_genes, pvalueCutoff = 1, pAdjustMethod = "none",qvalueCutoff = 1.0)
    
    if(orga_type == "mouse"){
      egmt <- clusterProfiler::enricher(unlist(list_of_genes_mouse_human), TERM2GENE=c1_hallmark_genes, universe = universe_mouse_human, pvalueCutoff = 1, pAdjustMethod = "none",qvalueCutoff = 1.0)
    }else{
      egmt <- clusterProfiler::enricher(unlist(list_of_genes), TERM2GENE=c1_hallmark_genes, universe = universe, pvalueCutoff = 1, pAdjustMethod = "none",qvalueCutoff = 1.0)
    }
    ...
    #TF prediction
    plotFunction<-function(){
      p<- ggplot2::ggplot(TFs_matrix_all_plot.melt.mean,aes(x=variable,y=mean,fill=condition)) +
        geom_bar(stat="identity",position = "dodge") +
        facet_wrap(~variable,scales = "free")
      print(p)
    }
    ...
    if(orga_type == "mouse"){
      ...
      TFtable <- pcaGoPromoter::primo(unlist(list_of_genes), inputType = "geneSymbol", org = "Mm")
      
      TFs <- stringr::str_split(TFtable$overRepresented[,4], "_")
      TFs <- do.call(rbind, TFs)[,1]
      TFs <- stringr::str_split(TFs, "::")
      ...
      TFtable <- pcaGoPromoter::primo(unlist(list_of_genes), inputType = "geneSymbol", org = "Hs")
    ...
    if(!is.null(TFs)){
      ...
      if(length(TFs) != 0L){
        ...
        TFs_matrix_all_plot <- normdata[,c("condition",TFS_intersect)]
        
        TFs_matrix_all_plot.melt <- melt(TFs_matrix_all_plot, id="condition")
        # calculate means
        require(dplyr)
        library(dplyr)
        TFs_matrix_all_plot.melt %>% group_by(condition,variable) -> TFs_matrix_all_plot.melt.mean
        ...
        condis <- unique(normdata$condition)
        TFs_matrix_all_plot.melt.mean$condition <- factor(TFs_matrix_all_plot.melt.mean$condition,levels = condis)
        
        # plot 1
        # ggplot(TFs_matrix_all_plot.melt.mean,aes(x=variable,y=mean,fill=condition)) +
        #   geom_bar(stat="identity",position = "dodge")
        #
        # ggplot(TFs_matrix_all_plot.melt.mean,aes(x=variable,y=mean,fill=condition)) +
        #   geom_bar(stat="identity",position = "dodge") +
        ...
        if(length(TFS_intersect) != 0L){
          ...
          TFs_matrix <- normdata[,c("condition",head(TFS_intersect,4))]
          
          TFs_matrix.melt <- melt(TFs_matrix, id="condition")
          # calculate means
          #require(dplyr)
          TFs_matrix.melt %>% group_by(condition,variable) -> TFs_matrix.melt.mean
          ...
          condis <- unique(normdata$condition)
          TFs_matrix.melt.mean$condition <- factor(TFs_matrix.melt.mean$condition,levels = condis)
          
          TF_plot <- ggplot2::ggplot(TFs_matrix.melt.mean,aes(x=variable,y=mean,fill=condition)) +
            geom_bar(stat="identity",position = "dodge") +
            facet_wrap(~variable,scales = "free")
    ...
    saveWorkbook(wb, paste("clusterProfiler/ClusterProfiler_",colnames(cluster_genes_original)[id_it],".xlsx",sep=""))
    dev.off()
...
plot_single_cluster<-function(igraph_object=igraph_object,
  ...
  if(length(cluster_name)>1){
    ...
    cluster_genes<-cluster_information[grepl(cluster_name,cluster_information$color),c("Gene")]
  }else{
    cluster_genes<-cluster_information[grepl(cluster_name,cluster_information$color),c("Gene")]
  }
  ...
  if(string_needs_to_be_redone ==TRUE){
    ...
    FromTo<-clust_fr_to
    #####################ACVHTUNG
    #FALL ABKL�REN WENN KEINE KNOWNS
    if(color_STRING %in% FromTo$col){
      ...
      keep_weil_hub_genes<-gene_conn_known[(gene_conn_known$Freq> no_strings_to_be_string_hub),] ### �nern zu absolute zahl!
...
####
merge_cluster<-function(data=clustered_heatmap_data,cluster_wanted=5){
  summary<-allargs()
  ...
  rnames$condition <- paste("cluster ", rnames$new,  " : ", rnames$x, " genes")
  
  
  names <- unique(rnames$condition)
  ...
  cluster_information$`clp$membership`<- cluster_information$clp2 # Anmerkung: c k�nnten im weiteren Verlauf ein Problem werden -> Zeilen l�schen?
  
  cluster_information$try <- cluster_information$try2 
  
  
  rnames$condition <- paste("cluster ", rnames$new, " ",rnames$cl.colour, " : ", rnames$x, " genes")
...
clusterCircos <- function(hm_vec, seg, range, y_comp){
  ...
  cluster_information <- cluster_information[!cluster_information$try=="waste",]
